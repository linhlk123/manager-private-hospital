...
public class LichSuLichHen extends JFrame {

    private CardLayout cardLayout;
    private JPanel contentPanel;

    public LichSuLichHen() throws SQLException, ClassNotFoundException {
    ...
    private JPanel createAppointmentMainPanel() {
        ...
        gbc.gridx = 2;
        searchPanel.add(ngayHenLabel, gbc);

        gbc.gridx = 3;
        searchPanel.add(ngayHenField, gbc);

        JLabel maBSLabel = new JLabel("Mã bác sĩ:");
        maBSLabel.setFont(labelFont);
        gbc.gridx = 4;
        searchPanel.add(maBSLabel, gbc);

        gbc.gridx = 5;
        searchPanel.add(mabsField, gbc);

        JLabel trangThaiLabel = new JLabel("Trạng thái:");
        trangThaiLabel.setFont(labelFont);
        gbc.gridx = 6;
        searchPanel.add(trangThaiLabel, gbc);

        gbc.gridx = 7;
        searchPanel.add(trangThaiBox, gbc);
    ...
    private List<String[]> searchAdvancedAppointments(String keyword, String ngayHen, String mabs, String trangThai)
        ...
        if (!keyword.isEmpty()) {
            sql.append(" AND (LOWER(MALICH) LIKE ? OR LOWER(MABN) LIKE ? OR LOWER(MABS) LIKE ? OR LOWER(NGAYDAT) LIKE ? OR LOWER(NGAYHEN) LIKE ? OR LOWER(DIADIEM) LIKE ? OR LOWER(TRIEUCHUNG) LIKE ?)");
            for (int i = 0; i < 7; i++) {
    ...
    private void showUpdateForm(String id, String ngayHen, String diaDiem, String trieuChung) {
        ...
        saveBtn.addActionListener(e -> {
            try {
                ...
                Logger.getLogger(LichSuLichHen.class.getName()).log(Level.SEVERE, null, ex);
            }
            dialog.dispose();
            ...
            btnDatLich.addActionListener(e -> {
                try {
                    String maBenhNhan = JOptionPane.showInputDialog(
                            null,
                            "Nhập mã bệnh nhân cần đặt lịch:",
                            "Đặt lịch khám",
                    ...
                    if (maBenhNhan != null && !maBenhNhan.trim().isEmpty()) {
                        ...
                        new AppointmentForm(maBenhNhan).setVisible(true);
                    } else {
