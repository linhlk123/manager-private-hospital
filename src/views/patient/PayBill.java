...
//            "Mã đơn thuốc", "Mã dược sĩ", "Mã bác sĩ", "Mã bệnh nhân", "File đơn thuốc", 
//            "Nội dung", "Ghi chú", "Ngày bán", "Thành tiền", "Trạng thái thanh toán"
//        });
//        
//        tableThuoc.getColumnModel().getColumn(9).setCellRenderer(new StatusCellRenderer());
//       
//
//        String sql = "SELECT MADT, MADS, MABS, MABN, FILEDONTHUOC, NOIDUNGDT, GHICHU, NGAYBAN, THANHTIEN, TRANGTHAITT FROM DONTHUOC_DONTHUOCYC";
...
//
//        String sql = "SELECT MAHDKB, MAKHAM, NGAYLAP, TONGTIEN, PHUONGTHUCTT, GHICHU, TRANGTHAITT FROM HOADON_KHAMBENH";
//
//        try (Connection conn = DBConnection.getConnection();
//             PreparedStatement stmt = conn.prepareStatement(sql);
//             ResultSet rs = stmt.executeQuery()) {
...
//
//        String sql = "SELECT MADTR, MAKHAM, MABS, PHONGDIEUTRI, PHUONGPHAPDT, NGAYTIEPNHAN, NGAYKTDK, TONGHDDT, KETQUADT, LUUY, TRANGTHAITT FROM DIEUTRI";
//
//        try (Connection conn = DBConnection.getConnection();
//             PreparedStatement stmt = conn.prepareStatement(sql);
//             ResultSet rs = stmt.executeQuery()) {
...
//                while (rs.next()) {
//                    doc.insertString(doc.getLength(), "Sản phẩm: " + rs.getString("MASP") + ", SL: " + rs.getInt("SOLUONG") + ", Trị giá: " + rs.getDouble("TRIGIA") + "\n", normalAttr);
//                }
...
public class PayBill extends JFrame {
    ...
    public void loadThuocTableData() throws SQLException, ClassNotFoundException {
        ...
        model.setColumnIdentifiers(new Object[] {
            "Mã đơn thuốc", "Mã dược sĩ", "Mã bác sĩ", "Mã bệnh nhân", "Giới tính bệnh nhân", "Lịch sử bệnh lý bệnh nhân",
