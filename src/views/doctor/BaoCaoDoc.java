...
public class BaoCaoDoc extends JFrame {
    private JPanel contentPanel;
    private CardLayout contentLayout;
    private JLabel khamBenhLabel, lichHenLabel, donThuocLabel, benhNhanLabel, hoaDonLabel;
    private String doctorId;

    public BaoCaoDoc(String doctorId) throws SQLException, ClassNotFoundException {
...
private JPanel createDoanhThuPanel() {
    ...
    for (int y = currentYear; y >= currentYear - 10; y--) yearCombo.addItem(y);

    // Chọn kiểu xem
    JComboBox<String> typeCombo = new JComboBox<>(new String[]{"By Month", "By Year"});
    ...
    Runnable updateChart = () -> {
        ...
        try (Connection conn = DBConnection.getConnection()) {
            if ("By Month".equals(type)) {
                // Medicine Invoice by month
                String sql1 = "SELECT EXTRACT(MONTH FROM NGAYBAN) AS THANG, SUM(THANHTIEN) AS DOANHTHU " +
                        "FROM DONTHUOC_DONTHUOCYC WHERE EXTRACT(YEAR FROM NGAYBAN) = ? " +
                ...
                while (rs1.next()) {
                    ...
                    double doanhThu = rs1.getDouble("DOANHTHU");
                    dataset.addValue(doanhThu, "Medicine Invoice", "Month " + thang);
                }

                // Medical Bill by month
                String sql2 = "SELECT EXTRACT(MONTH FROM NGAYLAP) AS THANG, SUM(TONGTIEN) AS DOANHTHU " +
                        "FROM HOADON_KHAMBENH WHERE EXTRACT(YEAR FROM NGAYLAP) = ? " +
                ...
                while (rs2.next()) {
                    ...
                    dataset.addValue(doanhThu, "Medical Bill", "Month " + thang);
                }
            } else {
                // Medicine Invoice by year
                String sql1 = "SELECT EXTRACT(YEAR FROM NGAYBAN) AS NAM, SUM(THANHTIEN) AS DOANHTHU " +
                        "FROM DONTHUOC_DONTHUOCYC GROUP BY EXTRACT(YEAR FROM NGAYBAN)";
                Statement st1 = conn.createStatement();
                ResultSet rs1 = st1.executeQuery(sql1);
                while (rs1.next()) {
                    int nam = rs1.getInt("NAM");
                    double doanhThu = rs1.getDouble("DOANHTHU");
                    dataset.addValue(doanhThu, "Medicine Invoice", String.valueOf(nam));
                }

                // Medical Bill by year
                String sql2 = "SELECT EXTRACT(YEAR FROM NGAYLAP) AS NAM, SUM(TONGTIEN) AS DOANHTHU " +
                        "FROM HOADON_KHAMBENH GROUP BY EXTRACT(YEAR FROM NGAYLAP)";
                Statement st2 = conn.createStatement();
                ResultSet rs2 = st2.executeQuery(sql2);
                while (rs2.next()) {
                    int nam = rs2.getInt("NAM");
                    double doanhThu = rs2.getDouble("DOANHTHU");
                    dataset.addValue(doanhThu, "Medical Bill", String.valueOf(nam));
                }
            }
        } catch (Exception e) {
            e.printStackTrace();
        ...
        String chartTitle = "Revenue ";
        if ("By Month".equals(type)) chartTitle += "by Month in " + yearCombo.getSelectedItem();
        else chartTitle += "by Year";
        JFreeChart chart = ChartFactory.createBarChart(
                chartTitle, 
                "Time", "Revenue", 
                dataset, 
        ...
        CategoryPlot plot = chart.getCategoryPlot();
        BarRenderer renderer = new BarRenderer();
        renderer.setSeriesPaint(0, new Color(56, 122, 130)); // Medicine Invoice
        renderer.setSeriesPaint(1, new Color(194, 224, 135)); // Medical Bill
        plot.setRenderer(renderer);
        plot.setBackgroundPaint(Color.WHITE);
        plot.setRangeGridlinePaint(Color.GRAY);
        chart.setBackgroundPaint(new Color(0xCDE8E5));
    ...
    yearCombo.addActionListener(e -> updateChart.run());
    typeCombo.addActionListener(e -> updateChart.run());
    updateChart.run();
