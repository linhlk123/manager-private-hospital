...
public class DoctorDashboard extends JFrame {
    private String currentDoctor, doctorId;
    private JPanel contentPanel;  // panel chứa các màn hình chức năng
    private CardLayout cardLayout;
    private JPanel sideMenu; 

    public DoctorDashboard(String hoTen, String maBS) throws SQLException, ClassNotFoundException {
        this.currentDoctor = hoTen;
        this.doctorId = maBS;

        System.out.println("Tên: " + hoTen);
        System.out.println("Mã bác sĩ: " + doctorId);
    ...
    private JPanel createMainFunctionPanel() {
        ...
        wrapper.add(createFunctionContainer("Hồ sơ bệnh nhân", "src/views/doctor/image/Profile_Pa.jpg"));
        wrapper.add(createFunctionContainer("Dịch vụ khám", "src/views/patient/image/service.png"));
        wrapper.add(createFunctionContainer("Hóa đơn", "src/views/patient/image/bill.jpg"));
        ...
        JLabel welcomeLabel = new JLabel("Chào mừng bác sĩ: " + currentDoctor, SwingConstants.CENTER);
    ...
    private void handleMenuClick(String title) throws SQLException, ClassNotFoundException {
        ...
        switch (title) {
            ...
            case "Lịch hẹn" -> {
                ...
                new HisAppointDoc(doctorId).setVisible(true);
            }
            case "Tra cứu thuốc" -> new ProductView().setVisible(true);
            case "Lịch sử khám" -> {
                System.out.println(doctorId);
                new HisMedExamDoc(doctorId).setVisible(true);
            }
            case "Hồ sơ bệnh nhân" -> {
                System.out.println(doctorId);
                new ProfilePatient(doctorId).setVisible(true);
            }
            case "Dịch vụ khám" -> {
                System.out.println(doctorId);
            ...
            case "Hóa đơn" -> {
                System.out.println(doctorId);
                new Bill(doctorId).setVisible(true);
            }
            case "Báo cáo thống kê" -> {
                System.out.println(doctorId);
                new BaoCao(doctorId).setVisible(true);
            }
   ...
   public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            try {
                ...
                String hoTen = "Nguyễn Ngọc Hân";
                String maBS = "U001";

                DoctorDashboard dashboard = new DoctorDashboard(hoTen, maBS);
